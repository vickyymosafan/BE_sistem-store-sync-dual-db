...
import { listAllPrices } from '../../../domain/usecases/listAllPrices';
import { toPriceWithProductDTO } from '../../../infra/mappers/priceMapper';

export async function listAllPricesHandler(
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> {
  try {
    ...
    const prices = await listAllPrices(priceRepository, storeId);
